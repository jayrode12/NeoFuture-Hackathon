...
export default function Login() {
  ...
  return (
    <div className="bg-surface font-body text-on-surface min-h-screen overflow-x-hidden">
      <div className="min-h-screen flex flex-col md:flex-row items-stretch overflow-x-hidden">
        ...
        <aside className="hidden md:flex md:w-5/12 lg:w-1/2 relative bg-primary-container overflow-hidden p-8 lg:p-16 flex-col justify-between">
          <div className="relative z-10">
            <Link to="/" className="block text-3xl font-headline font-bold tracking-tighter bg-gradient-to-br from-white to-indigo-300 bg-clip-text text-transparent mb-12">
              Invisible India
            </Link>
          ...
          <div className="relative z-10 flex gap-4 backdrop-blur-md bg-white/5 p-6 rounded-2xl border border-white/10 max-w-fit">
            <div className="flex -space-x-3">
              {[1, 2, 3].map((i) => (
                <img key={i} className="w-10 h-10 rounded-full border-2 border-primary-container object-cover" src={`https://i.pravatar.cc/100?img=${i+10}`} alt="user" />
              ))}
            </div>
        ...
        <main className="flex-1 flex flex-col justify-center px-6 py-12 md:px-10 lg:px-20 bg-surface-bright relative overflow-hidden">
          ...
          <form className="max-w-md w-full mx-auto space-y-7 relative z-10" onSubmit={handleLogin}>
            <div className="md:hidden mb-12">
              <Link to="/" className="block text-2xl font-headline font-bold tracking-tighter bg-gradient-to-br from-indigo-900 to-indigo-500 bg-clip-text text-transparent">
                Invisible India
              </Link>
            </div>
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                <label className="block text-sm font-label font-semibold text-on-surface-variant ml-1" htmlFor="email">Email Address</label>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center ml-1">
                  <label className="block text-sm font-label font-semibold text-on-surface-variant" htmlFor="password">Password</label>
          ...
          <footer className="mt-auto pt-16 hidden md:block border-t border-outline-variant/10">
            <div className="flex justify-between items-center text-outline-variant text-xs">
              <p>© 2024 Invisible India. Empowering the Resilient Horizon.</p>
