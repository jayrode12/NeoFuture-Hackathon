...
const Home = () => {
  return (
    <div className="bg-surface text-on-surface font-body antialiased min-h-screen">
      {/* Top Navigation */}
      <header className="bg-surface/80 backdrop-blur-xl sticky top-0 z-50 transition-colors duration-300 border-b border-surface-container">
        <nav className="flex justify-between items-center px-8 py-6 w-full max-w-7xl mx-auto">
          <div className="text-2xl font-bold tracking-tighter text-indigo-950 font-headline">Invisible India</div>
          <div className="hidden md:flex gap-8 items-center">
            <a className="text-indigo-900 border-b-2 border-indigo-900 pb-1 font-headline font-bold tracking-tight" href="#">Schemes</a>
            <a className="text-slate-600 font-medium hover:text-indigo-700 transition-colors duration-300" href="#">About Us</a>
            <a className="text-slate-600 font-medium hover:text-indigo-700 transition-colors duration-300" href="#">Contact</a>
            <div className="flex gap-4 ml-4">
              <button className="text-indigo-900 font-bold px-4 py-2">Hindi/English</button>
              <Link to="/login" className="bg-primary-container text-white px-8 py-3 rounded-full hover:scale-95 transition-all duration-200">Login</Link>
            </div>
          </div>
        </nav>
      </header>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
              ...
              <div className="bg-surface-container-lowest p-10 rounded-lg editorial-shadow transform hover:-translate-y-2 transition-transform duration-300">
                ...
                <button className="w-full py-4 rounded-full bg-surface-container font-bold text-indigo-900 hover:bg-indigo-900 hover:text-white transition-colors">Details</button>
              </div>
              {/* Card 2 */}
              <div className="bg-surface-container-lowest p-10 rounded-lg editorial-shadow transform hover:-translate-y-2 transition-transform duration-300">
      ...
      {/* Footer */}
      <footer className="bg-surface-container-low py-16 px-8 border-t border-surface-container">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
          <div className="md:col-span-2">
            <div className="text-2xl font-bold text-indigo-950 mb-6">Invisible India</div>
            <p className="text-slate-500 max-w-sm mb-8 leading-relaxed">We are building the trust layer for India's informal workforce, providing tools for economic resilience and digital dignity.</p>
            <p className="text-sm text-slate-500">© 2024 Invisible India. Secure & Private.</p>
          </div>
          <div>
            <h5 className="font-headline font-bold text-indigo-950 mb-6 uppercase text-xs tracking-widest">Platform</h5>
            <ul className="space-y-4">
              <li><a className="text-slate-500 hover:text-indigo-600 transition-colors" href="#">Government Schemes</a></li>
              <li><a className="text-slate-500 hover:text-indigo-600 transition-colors" href="#">Worker Profiles</a></li>
              <li><a className="text-slate-500 hover:text-indigo-600 transition-colors" href="#">Trust Score</a></li>
            </ul>
          </div>
          <div>
            <h5 className="font-headline font-bold text-indigo-950 mb-6 uppercase text-xs tracking-widest">Company</h5>
            <ul className="space-y-4">
              <li><a className="text-slate-500 hover:text-indigo-600 transition-colors" href="#">Help Center</a></li>
              <li><a className="text-slate-500 hover:text-indigo-600 transition-colors" href="#">Privacy Policy</a></li>
              <li><a className="text-slate-500 hover:text-indigo-600 transition-colors" href="#">Contact Support</a></li>
            </ul>
          </div>
        </div>
      </footer>
